/**
 * An UnsafeByteBuffer is effectively a ByteBuffer that does not own the memory it reads/writes from.
 *
 * THIS CLASS DOES NOT OWN THE BYTE ARRAY PASSED INTO THE CONSTRUCTOR, AND WILL NOT DELETE IT.
 */

#ifndef _UnsafeByteBuffer_H_
#define _UnsafeByteBuffer_H_
...
public:
	UnsafeByteBuffer(uint8_t* arr, uint32_t size);
	~UnsafeByteBuffer() = default;

    uint32_t bytesRemaining() const; // Number of uint8_ts from the current read position till the end of the buffer
	...
	void getBytes(uint8_t* dest, uint32_t len) const; // Absolute read into array buf of length len
	...
	void put(UnsafeByteBuffer* src); // Relative write of the entire contents of another UnsafeByteBuffer (src)
...
private:
	...
	template<typename T> void append(T data) {
		...
		if (size() < (wpos + s)) {
            throw std::runtime_error("append() failed! Cannot resize UnsafeByteBuffer!");
        }
	...
	template<typename T> void insert(T data, uint32_t index) {
		if ((index + sizeof(data)) > size()) {
            throw std::runtime_error("insert() failed! Cannot resize UnsafeByteBuffer!");
		}
